...
`Checkpoint Thumbnail Exporter` is a small utility node for creating missing checkpoint thumbnails for `OGN-ModelManager` from image folders generated by HandpickerSuite / GM Image Saver.
...
↓
Checkpoint-specific image folders are accumulated
↓
...
Root folder containing checkpoint-specific image folders.

Leave empty to use the ComfyUI output folder.
